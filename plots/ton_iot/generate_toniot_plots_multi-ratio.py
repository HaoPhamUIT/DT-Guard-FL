#!/usr/bin/env python3
"""
generate_toniot_plots_multi-ratio.py — Multi-ratio Figures for DT-Guard (ToN-IoT Dataset)
==========================================================================================
Multi-ratio version of generate_toniot_plots.py.
Produces figures showing 4 attack ratios: 10%, 20%, 40%, 50%.

  Fig. 2 (Gap 1) — Accuracy + Detection Rate @ 4 ratios (10%,20%,40%,50%) [2×4 grid]
  Fig. 3 (Gap 2) — FPR heatmap @ 4 ratios (10%,20%,40%,50%)                [1×4 grid]

Usage:
    python generate_toniot_plots_multi-ratio.py              # all figures
    python generate_toniot_plots_multi-ratio.py --only fig2 # Gap 1 only
    python generate_toniot_plots_multi-ratio.py --only fig3 # Gap 2 only
    python generate_toniot_plots_multi-ratio.py --fmt png --dpi 600
"""
...
def plot_fig2(fmt='png'):
    ...
    for idx, ratio in enumerate(available_ratios):
        ...
        ax_acc.set_title(f'({chr(97+idx*2)}) {ratio} Malicious [ToN-IoT]', fontsize=7, fontweight='bold', pad=2)
        ...
        ax_det.set_title(f'({chr(97+idx*2+1)}) {ratio} Malicious [ToN-IoT]', fontsize=7, fontweight='bold', pad=2)
        ax_det.set_ylim(0, 109)

        # Remove x-axis labels for all but last row to save space
...
def plot_fig3(fmt='png'):
    ...
    for idx, ratio in enumerate(available_ratios):
        ...
        ax.set_title(f'{ratio} [ToN-IoT]', fontsize=8, fontweight='bold')
    ...
    plt.suptitle('False Positive Rate across Attack Ratios [ToN-IoT]', fontsize=9, fontweight='bold', y=0.98)
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate multi-ratio figures for DT-Guard (ToN-IoT dataset)')
    ...
    print("┌──────────────────────────────────────────────────────────┐")
    print("│  DT-Guard — Multi-Ratio ToN-IoT Figures                 │")
