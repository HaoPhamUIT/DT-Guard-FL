...
def run_dtguard(attack_type, X_clients, y_clients, X_test, y_test,
    ...
    # Reuse cached verifier (TabDDPM trained once)
    if cached_verifier is not None:
        ...
        tabddpm_gen.train_gan(benign_X, benign_y, device=device)
        verifier = DigitalTwinVerifier(tabddpm_gen, threshold=DT_THRESHOLD,
...
def main():
    ...
    # ---- Pre-train TabDDPM ONCE for DT-Guard ----
    # TabDDPM is trained on benign client data only — same data for every attack
    # This is a legitimate optimization: TabDDPM models the benign data distribution,
    # which does not change across attacks.
    print("Pre-training TabDDPM for DT-Guard (one-time)...", end=" ", flush=True)
    t_gen = time.time()
    tabddpm_gen = TabDDPMChallengeGenerator(
        input_dim=input_dim, n_classes=num_classes,
        T=200, d_hidden=256, n_epochs=TABDDPM_EPOCHS, batch_size=512)
    ...
    # Subsample for TabDDPM training speed (15K is sufficient to learn distribution)
    X_all_benign = np.vstack(benign_X)
    y_all_benign = np.concatenate(benign_y)
    if len(X_all_benign) > 15000:
        idx = np.random.choice(len(X_all_benign), 15000, replace=False)
        X_all_benign = X_all_benign[idx]
        y_all_benign = y_all_benign[idx]
    tabddpm_gen.train_gan(X_all_benign, y_all_benign, device=device)
    cached_verifier = DigitalTwinVerifier(tabddpm_gen, threshold=DT_THRESHOLD,
                                          challenge_samples=CHALLENGE_SAMPLES)
