#!/usr/bin/env python3
"""
EXPERIMENT 4: NON-IID ROBUSTNESS - ToN-IoT Dataset
==========================================
...
This mirrors experiments/thesis/run_experiment4_noniid.py but uses ToN-IoT dataset.
"""
...
def run_one(defense_name, attack_type,
    ...
    if verifier is not None:
        ...
        # Adaptive threshold: lower for high α (near-IID)
        if alpha >= 5.0:
            verifier.threshold = 0.50
        elif alpha >= 1.0:
...
def main():
    start = datetime.now()
    print("=" * 100)
    print("  EXPERIMENT 4: NON-IID ROBUSTNESS — ToN-IoT Dataset")
    print("=" * 100)
    ...
    results_noatk = {d: {} for d in DEFENSES}
    results_lie   = {d: {} for d in DEFENSES}
    ...
    # =========================================================================
    print("\n" + "=" * 100)
    print("  TABLE A: FALSE POSITIVE RATE (No Attack) - ToN-IoT — lower is better")
    print("=" * 100)
    hdr = f"  {'Defense':<15}"
    for a in ALPHAS:
        hdr += f"  α={a:<5}"
    ...
    # =========================================================================
    # TABLE B: Accuracy under BACKDOOR at different Non-IID levels
    # =========================================================================
    print("\n" + "=" * 100)
    print("  TABLE B: ACCURACY under BACKDOOR Attack - ToN-IoT — higher is better")
    print("=" * 100)
    hdr = f"  {'Defense':<15}"
    for a in ALPHAS:
        hdr += f"  α={a:<5}"
    ...
    # =========================================================================
    # TABLE C: Detection Rate under BACKDOOR
    # =========================================================================
    print("\n" + "=" * 100)
    print("  TABLE C: DETECTION RATE under BACKDOOR Attack - ToN-IoT — higher is better")
    print("=" * 100)
    hdr = f"  {'Defense':<15}"
    for a in ALPHAS:
        hdr += f"  α={a:<5}"
    ...
    with open(results_dir / 'exp4_noniid_toniot.json', 'w') as f:
        json.dump(save_data, f, indent=2, default=float)
    print(f"\n  Results saved to {results_dir / 'exp4_noniid_toniot.json'}")

    print(f"\n  ✅ Experiment 4 (ToN-IoT) done in {elapsed}")
